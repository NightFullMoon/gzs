<template>
  <div class="i-auto-complete">
    ...
    <div v-show="mShowPanel" class="complete-panel">
      <!-- loading槽 -->
      <div v-show="mIsLoading" class="slot-loading">
        <slot name="loading">
          <div class="loading-wrap">
            <i class="fa fa-spinner icon-loading" aria-hidden="true"></i>
          </div>
        </slot>
      </div>

      <!-- noResult槽 -->
      <div v-show="renderSuggests.length<1 && !mIsLoading" class="slot-no-result">
        <slot name="noResult">
          <p class="not-result-text">未找到</p>
        </slot>
      </div>

      <!-- 默认槽，用来放渲染结果 -->
      <div v-show="0<renderSuggests.length" class="slot-default">
...
export default {
  ...
  methods: {
    ...
    handlePromise: function(promise) {
      var that = this;
      // 触发补全的话，就将这两个都设置为空，以保证能触发真正的重计算
